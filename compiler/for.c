static void retrophies_parser_parsefor(retrophies_parser_t* self, retrophies_parser_subroutine_t* sub)
{
  ...
  const retrophies_parser_local_t* var = retrophies_parser_findlocal(sub, name.hash);
  ...
  retrophies_parser_emit(self, RETROPHIES_INSN_SETLOCAL, index);
  ...
  {
    if (var->name.length != name.lexeme.length || memcmp(var->name.chars, name.lexeme.chars, var->name.length) != 0)
    {
      retrophies_parser_error(self, name.line, "Wrong control variable: %.*s, should be %.*s", name.lexeme.chars, name.lexeme.length, var->name.chars, var->name.length);
      /* never returns */
    }
  }
